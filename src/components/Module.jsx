...
function Module() {
  ...
  if (!course) {
    return (
      <p className="text-center text-gray-600 mt-10">No course selected.</p>
    );
  }

  // Example structure for chapters
  const chapters = [
    {
      title: "Chapter 01",
      lectureNotes: [
    ...
    <div className="min-h-screen p-8 bg-gray-100 flex flex-col items-center">
      ...
      <div className="text-left w-full max-w-4xl">
        ...
        {chapters.map((chapter, index) => (
          <div key={index} className="bg-white shadow-lg rounded-lg p-6 w-full mb-6">
            <h2 className="text-2xl font-semibold text-gray-800">{chapter.title}</h2>

            {/* Lecture Notes */}
            <div className="mt-6">
              <h3 className="text-xl font-semibold text-gray-800">📖 Lecture Notes</h3>
              <ul className="list-disc list-inside text-gray-700 mt-2">
                {chapter.lectureNotes.map((note, index) => (
                  <li key={index}>
                    <a href={note.link} className="text-blue-500 hover:underline">
                      {note.title}
                    </a>
                  </li>
                ))}
            ...
            {/* YouTube Videos */}
            <div className="mt-6">
              <h3 className="text-xl font-semibold text-gray-800">🎥 YouTube Videos</h3>
              <ul className="list-disc list-inside text-gray-700 mt-2">
            ...
            <div className="mt-6">
              <h3 className="text-xl font-semibold text-gray-800">📝 Assignment Submission</h3>
              <a
